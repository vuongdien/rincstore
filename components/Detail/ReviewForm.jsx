          ...
          {!ratingDoc?.exists() ? (
            <div className="rating-selector">
              <div onClick={() => rate("exceptional", user.uid, gameRef)}>
                <Image
                  src="/icons/ratings/exceptional.png"
                  width={35}
                  height={35}
                  quality="35"
                  alt="rating-exceptional"
                />
                <p>Exceptional</p>
              </div>
              <div onClick={() => rate("recommended", user.uid, gameRef)}>
                <Image
                  src="/icons/ratings/recommended.png"
                  width={35}
                  height={35}
                  quality="35"
                  alt="rating-recommended"
                />
                <p>Recommended</p>
              </div>
              <div onClick={() => rate("meh", user.uid, gameRef)}>
                <Image
                  src="/icons/ratings/meh.png"
                  width={35}
                  height={35}
                  quality="35"
                  alt="rating-meh"
            ...
            ratingData && (
              <div className="rating-selector">
                <div
                  className="rated"
                  onClick={() => rate(ratingData?.rating, user.uid, gameRef)}
                >
                  <Image
                    src={`/icons/ratings/${ratingData.rating}.png`}
                    width={35}
                    height={35}
                    quality="35"
                    alt={`rating-${ratingData.rating}`}
                  />
          ...
          {!reviewDoc?.exists() ? (
            ...
            <form onSubmit={updateReview} className="review-viewer">
              <textarea
                ...
                placeholder="writer your review..."
                disabled={!reviewEdit}
                value={reviewEdit ? reviewController : reviewData?.review}
                onChange={(e) => setReviewController(e.target.value)}
              />
              {reviewEdit ? (
                <div>
                  <button type="submit">
                    Submit your updated review <FaCheck />
                  </button>
                  <button
                    type="button"
                    onClick={(e) => {
